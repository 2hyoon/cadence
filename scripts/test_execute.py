...
def executor(tmp_project, phase_dir):
    """StepExecutor instance for tests. git calls must be mocked separately."""
    with patch.object(ex, "ROOT", tmp_project):
        inst = ex.StepExecutor("0-mvp")
    # Reset internal paths to be based on tmp_project
...
class TestJsonHelpers:
    ...
    def test_save_ensures_ascii_false(self, tmp_path):
        # Uses non-ASCII (Korean) on purpose: this verifies ensure_ascii=False
        # keeps multibyte characters literal instead of escaping them as \uXXXX.
        p = tmp_path / "test.json"
...
class TestLoadGuardrails:
    ...
    def test_empty_project(self, tmp_path):
        with patch.object(ex, "ROOT", tmp_path):
            # static-like behavior that needs no executor, so a bare instance
...
# ---------------------------------------------------------------------------
# progress_indicator (= former Spinner)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# _check_blockers (= former main() error/blocked check)
# ---------------------------------------------------------------------------
