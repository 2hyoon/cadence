...
def progress_indicator(label: str):
    """Terminal progress indicator. Use as a with statement; read elapsed time via .elapsed."""
    frames = "◐◓◑◒"
...
class StepExecutor:
    ...
    def _build_preamble(self, guardrails: str, step_context: str,
        ...
        return (
            ...
            f"## Working Rules\n\n"
            f"1. Review the code written in previous steps and keep it consistent.\n"
            f"2. Do only the work specified in this step. Do not create extra features or files.\n"
            f"3. Do not break existing tests.\n"
            f"4. Run the AC (Acceptance Criteria) verification yourself.\n"
            f"5. Update the status of this step in /phases/{self._phase_dir_name}/index.json:\n"
            f"   - AC passes → \"completed\" + a one-line summary of this step's deliverables in the \"summary\" field\n"
            f"   - Still failing after {self.MAX_RETRIES} fix attempts → record \"error\" + \"error_message\"\n"
            f"   - If user intervention is needed (API keys, auth, manual setup, etc.) → record \"blocked\" + \"blocked_reason\", then stop immediately\n"
    ...
    def _execute_single_step(self, step: dict, guardrails: str) -> bool:
        """Run a single step (with retries). Returns True on completion, False on failure/block."""
        step_num, step_name = step["step"], step["name"]
